---
id: loop-runtime-unit-tests-retry
level: task
title: "Loop runtime unit tests (retry, block, complete)"
short_code: "KAT-T-0111"
created_at: 2026-05-09T18:00:00+00:00
updated_at: 2026-05-11T13:35:05.175523+00:00
parent: KAT-I-0003
blocked_by: [KAT-T-0109]
archived: false
...
## Goal
Cover every loop outcome with a stub `GateAdapter` and `FakeDispatcher`, no filesystem.

## Deliverables
- New file: `/Users/danielcassil/Code/katana/tests/workflow/loop.test.ts`.
...
## Do / Don't
- DO use vitest's `vi.spyOn` only for the threading test.
- DON'T touch the real gate engine, dispatcher adapters, or filesystem.
- DON'T assert on `updated_at` / `file_hash` (the in-memory store doesn't recompute them).

## Acceptance
- [ ] File exists at the exact path.
- [ ] `npx vitest run tests/workflow/loop.test.ts` passes.
- [ ] All 6 cases above pass.

## Dependencies
- KAT-T-0109 (loop runtime).